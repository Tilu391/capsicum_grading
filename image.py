...
if uploaded_file is not None and camera_image is not None:
    ...
    if image:
        ...
        # Ensure the image exists before running YOLOv5 detection
        if os.path.exists(image_path):
            with st.spinner("Running YOLOv5 detection..."):
                # YOLOv5 detection command
                command = [
                    "python", "yolov5/detect.py", "--weights", "yolov5/best.pt", "--img", "640", "--conf", "0.10",
...
def grade_capsicum(capsicum_height, capsicum_width, health_status):
    if health_status == "healthy":
        if capsicum_height > 230 and capsicum_width > 220:
...
def make_background_white(image_path, detections_path, output_path):
    ...
    for (x1, y1, x2, y2) in bounding_boxes:
       ...
       health_status = "healthy"  # Assume healthy unless determined otherwise
       ripeness_label = classify_capsicum(image, (x1, y1, x2, y2))
       grade = grade_capsicum(capsicum_height, capsicum_width, health_status)
